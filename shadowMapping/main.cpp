...
//function updateLight(time, posx, posy, posz, dirx, diry, dirz, upx, upy, upz, colr, colg, colb, cola, intensity)
...
int main(void)
{
    ...
    GLuint vs = generateShader (luaHandler.getGlobalString("vertexShader"), GL_VERTEX_SHADER);
	GLuint fs = generateShader (luaHandler.getGlobalString("fragmentShader"), GL_FRAGMENT_SHADER);
	GLuint svs = generateShader(luaHandler.getGlobalString("skyboxVertexShader"), GL_VERTEX_SHADER);
	GLuint sfs = generateShader(luaHandler.getGlobalString("skyboxFragmentShader"), GL_FRAGMENT_SHADER);
	GLuint hvs = generateShader(luaHandler.getGlobalString("shadowVertexShader"), GL_VERTEX_SHADER);
	GLuint hfs = generateShader(luaHandler.getGlobalString("shadowFragmentShader"), GL_FRAGMENT_SHADER);
	GLuint dvs = generateShader(luaHandler.getGlobalString("debugQuadVertexShader"), GL_VERTEX_SHADER);
	GLuint dfs = generateShader(luaHandler.getGlobalString("debugQuadFragShader"), GL_FRAGMENT_SHADER);
    ...
    glAttachShader(shadowProgramme, hvs);
    glAttachShader(shadowProgramme, hfs);
    glLinkProgram(shadowProgramme);

	GLuint debugQuadProgramme = glCreateProgram();
    glAttachShader(debugQuadProgramme, dvs);
    glAttachShader(debugQuadProgramme, dfs);
    glLinkProgram(debugQuadProgramme);
